...
server <- function (input, output, session) {
  ...
  output$mdi_plot <- renderPlotly({
    ...
    p <- ggplot(df_grouped, aes(x = interaction(age,tx), y = index, color = sex, shape = sex), fill = interaction(sex, age))+
      geom_point(data = df3, aes(x = interaction(age,tx), y = index, color = sex), size = 2, position = position_dodge(.5))+
    ...
    if (input$p_vals){p <- p + stat_compare_means(data = df3, method = 't.test', aes(group = sex), label = 'p.signif')}
  ...
  output$mdi_genes <- renderDT(
    ...
    datatable(df4, options = list(sDom  = '<"top"><"bottom">ip'))
  )
